...
export async function PATCH(req: Request, ctx: { params: { userId: string } | Promise<{ userId: string }> }) {
  ...
  if (!body.userStatus)
    return NextResponse.json({ error: "userStatus is required." }, { status: 400 })
  ...
  if (guard.userId === userId && body?.userStatus === "DISABLED")
    return NextResponse.json({ error: "Admin cannot disable himself." }, { status: 400 })
  ...
  if (currentStatus === "UNREGISTERED")
    return NextResponse.json({ error: "UNREGISTERED user approved only through User Approvals." }, { status: 400 })

  if ((body as any).userStatus === "UNREGISTERED")
    return NextResponse.json({ error: "Invalid userStatus for this endpoint." }, { status: 400 })
